...
def generate_detector_configs(
    num_detectors: int = 6,
) -> List[BaseContentDetectionConfig]:
    """Generate detector configurations for multiple regex detectors"""
    return [
        BaseContentDetectionConfig(
            base_url=f"http://regex-detector-{i}-route-test.apps.rosa.trustyai-mac.bd9q.p3.openshiftapps.com",
            detector_id=f"regex-{i}",
...
async def run_single_test(adapter, case: TestCase) -> TestResult:
    ...
    try:
        ...
        params = {"detector_params": case.detector_params}
        if case.expected_detector:
            params["expected_detector"] = case.expected_detector
...
async def load_test(
    ...
    config = ContentDetectionConfig(detectors=generate_detector_configs(num_detectors))
    ...
    test_cases = [
        TestCase(
            ...
            detector_params={"regex": ["email"]},
            expected_detector=None,  # Don't specify expected detector since multiple might respond
        ),
        TestCase(
    ...
    print(f"\nStarting Multi-Detector Regex Test at {datetime.now()}")
...
def print_test_summary(
    ...
    # Calculate and print detector-specific stats
    detector_stats = {}
    for r in successful:
        if r.detector_id:
    ...
    # Save results to CSV with test case information
    with open("results_multi_regex_detector.csv", "w") as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run multi-detector regex load test")
    parser.add_argument(
