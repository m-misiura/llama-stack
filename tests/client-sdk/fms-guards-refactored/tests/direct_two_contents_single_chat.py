...
async def test_mixed_detectors():
    """Test content and chat detectors using their specific APIs"""
    # Configure content detector (HAP)
    ...
    # Configure regex detector
    regex_config = ContentDetectorConfig(
    ...
    # Configure chat detector (Granite)
    ...
    try:
        ...
        regex_adapter = await get_adapter_impl(regex_config)
        ...
        class MockShieldStore:
            async def get_shield(self, shield_id: str) -> Shield:
                ...
                elif shield_id == "test-shield-regex":
                    return Shield(
        ...
        regex_adapter.shield_store = MockShieldStore()
        ...
        print("\nRunning Mixed Detectors API Tests")
        print("==============================")
        ...
        for case in test_cases:
            ...
            responses = []
            if case["use_detector"] in ["content", "all"]:
            ...
            if case["use_detector"] in ["regex", "all"]:
                response = await regex_adapter.run_shield("test-shield-regex", messages)
                if response.violation:
                    responses.append(response)

            if case["use_detector"] in ["chat", "all"]:
        ...
        if regex_adapter:
            await regex_adapter.shutdown()
        if chat_adapter:
            await chat_adapter.shutdown()


if __name__ == "__main__":
    asyncio.run(test_mixed_detectors())
