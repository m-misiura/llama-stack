...
class ContentDetector(BaseDetector):
    ...
    def __init__(self, config: ContentDetectorConfig) -> None:
        if not isinstance(config, ContentDetectorConfig):
            raise ValueError("Config must be an instance of ContentDetectorConfig")
        ...
        try:
            ...
            if self.config.use_orchestrator_api:
                ...
                if isinstance(response, list) and response:
                    detections = (
                        response[0] if isinstance(response[0], list) else [response[0]]
                    )
    ...
    async def _run_shield_impl(
        ...
        try:
            ...
            for content in contents:
                logger.debug(
                    f"Checking content: {content[:100]}..."
                )  # Log first 100 chars
